...
//
     ...
     * @returns {*[]}
     */
    /*util_makeArray : function (targetList, resultList, isToastDefault=false) {
        if($A.util.isEmpty(targetList) === false) {
            targetList = $A.util.isArray(targetList) ? targetList : [targetList];

            targetList.forEach((myCheck) => {
                myCheck.get('v.checked') && resultList.push(myCheck.get('v.value'));
            });

            resultList = resultList.reduce((prev, curr) => {
                (!prev.includes(curr)) && prev.push(curr);

                return prev;
            }, []);

            if($A.util.isEmpty(resultList)) {
                (isToastDefault) && this.gfn_toast('resultList is empty.', 'w');
                return [];
            }
        }
        else {
            (isToastDefault) && this.gfn_toast('targetList is empty.', 'w');
            return [];
        }
    ...
    gfn_search : function(component, pageSize, pageNumber, tableId, apexCallMethodName){
        ...
        this.apex(
            ...
            $A.util.isEmpty(resData.recordList) && self.gfn_toast('No Data Found.', 'w');
            // ===============================================
    ...
    gfn_callPageFrame : function(component, result, size, tableId) {
        this.gfn_log(component, 'lacComBase.callPageFrame' , {'설명' : 'table의 sortData 메소드 호출'});
